...
(function (global) {
	...
	CarDodge.prototype.tryHandleChat = function (msg) {
		...
		} else if (cmd === "c") {          // c|gameId|tag|x|alive
			this._applyCar(parts[1], parts[2], (parseInt(parts[3], 10) || 0) / 1000, parts[4] === "1");
		} else if (cmd === "x") {          // x|gameId|tag|survivedMs
			this._applyCrash(parts[1], parts[2], parseFloat(parts[3]) || 0);
	...
	CarDodge.prototype._hostSpawnTick = function (now) {
		...
		var interval = clamp(900 - elapsed / 90, 360, 900);
		this._nextSpawnAt = now + interval;
		if (this.obstacles.length > 60) return;

		var speed = clamp(0.30 + elapsed / 90000, 0.30, 0.78);
		var o = {
			id: this._selfTag() + "-" + (this._seq++),
			x: rand(ROAD_L + OBST * 0.6, ROAD_R - OBST * 0.6),
			bornAt: now, speed: speed, kind: pick(KINDS)
		};
	...
	CarDodge.prototype._applyCar = function (gameId, tag, x, alive) {
		...
		p.lastSeen = Date.now();
		if (!alive && p.alive) { p.alive = false; }
	};

	CarDodge.prototype._applyCrash = function (gameId, tag, survivedMs) {
		if (gameId !== this.gameId) return;
		var p = this.players[tag];
		if (!p) return;
		if (p.alive) {
	...
	CarDodge.prototype._crashOwn = function (now) {
		...
		this.sendSync("x|" + this.gameId + "|" + this._selfTag() + "|" + Math.round(p.survivedMs));
		this._flash("💥 you crashed! survived " + (p.survivedMs / 1000).toFixed(1) + "s");
	};

	CarDodge.prototype._boom = function (x) {
		for (var i = 0; i < 18; i++) {
			var a = rand(0, Math.PI * 2), sp = rand(0.005, 0.022);
			this.particles.push({
	...
	CarDodge.prototype._sortedRanking = function () {
		...
		arr.sort(function (a, b) {
			...
			return (b.survivedMs - a.survivedMs) || (a.tag < b.tag ? -1 : 1);
		});
		return arr;
	};
	CarDodge.prototype._encodeRanking = function () {
		...
		for (var i = 0; i < arr.length; i++) {
			out.push(arr[i].tag + ":" + Math.round(arr[i].survivedMs) + ":" + (arr[i].alive ? 1 : 0) + ":" + sanitize(arr[i].name));
		}
		return out.join(";");
	};
	CarDodge.prototype._decodeRanking = function (str) {
		...
		for (var i = 0; i < entries.length; i++) {
			...
			if (!f[0]) continue;
			var p = this.players[f[0]] || this._addPlayer(f[0], f[3], this._hueOf(f[0]));
			if (!p) continue;
			p.survivedMs = parseInt(f[1], 10) || 0;
			p.alive = f[2] === "1";
			if (f[3]) p.name = sanitize(f[3]);
		}
	};
	...
	CarDodge.prototype._update = function (now) {
		...
		if (me && me.isOwn) {
			...
			if (me.alive && now - this._lastCarBcast > 66) {
				...
				this.sendSync("c|" + this.gameId + "|" + this._selfTag() + "|" + Math.round(me.x * 1000) + "|1|" + this._selfTag());
			}
		}
		...
		// my own collision (I'm the authority on my own crash)
		if (me && me.isOwn && me.alive && now >= this.startTime) {
			if (this._hitsObstacle(me.x, now)) this._crashOwn(now);
		}
		...
		var racers = this._roster().filter(function (q) { return true; });
	...
	CarDodge.prototype._hitsObstacle = function (carX, now) {
		var cw = CAR_W * 0.72, ch = CAR_H * 0.72;
		var cl = carX - cw / 2, cr = carX + cw / 2, ct = CAR_Y - ch / 2, cb = CAR_Y + ch / 2;
		for (var i = 0; i < this.obstacles.length; i++) {
			var o = this.obstacles[i];
			var oy = this._obstacleY(o, now);
			var ow = OBST * 0.7, oh = OBST * 0.7;
			if (cl < o.x + ow / 2 && cr > o.x - ow / 2 && ct < oy + oh / 2 && cb > oy - oh / 2) return true;
		}
		return false;
	};
	...
	CarDodge.prototype._draw = function (now) {
		...
		for (var j = 0; j < arr.length; j++) this._drawCar(ctx, arr[j], w, h);
		this._drawParticles(ctx, w, h);

		// countdown + HUD overlay on the canvas
	...
	CarDodge.prototype._drawCar = function (ctx, p, w, h) {
		...
		if (!arr.length) {
			...
			for (var i = 0; i < arr.length; i++) {
				...
				li.innerHTML = '<span class="cd-rank">' + medal + '</span>' +
					...
					'<span class="cd-time">' + (p.survivedMs / 1000).toFixed(1) + 's</span>';
				ol.appendChild(li);
			}
		}
